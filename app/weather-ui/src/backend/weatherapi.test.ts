...
const SAMPLE_API_RESPONSE = {
  ...
  hourly: {
    ...
    temperature_2m: [
      32.5, 32.1, 30.9, 28.7, 27.4, 26.4, 26, 26.1, 26, 24.7, 24.2, 23.8, 24,
      23.3, 24.1, 25.7, 27.2, 28.6, 30.7, 31.6, 32.3, 33.4, 33.5, 33.5,
    ],
  },
};
...
it('throws error when response is not 200', async () => {
  ...
  await expect(
    fetchWeatherData(axios, WEATHER_API_URL, '0.0', '0.0')
  ).rejects.toThrow();
});
...
it('gets low temp', () => {
  const response: CurrentWeatherApiResponse =
    currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
